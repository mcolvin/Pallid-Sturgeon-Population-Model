...
inits_ind<- function(input=input)
	...
	# INITIALIZE AT T=0
	viableGam<- as.matrix(cbind(
		rtriangle(input$nyears, input$viableGam_rng[1],input$viableGam_rng[2] ,input$viableGam),
		runif(1, input$viableGam_rng[1],input$viableGam_rng[2])))
	S0<-as.matrix(cbind(
		rtriangle(input$nyears,input$S0_rng[1],input$S0_rng[2],input$S0),
		runif(input$nyears,input$S0_rng[1],input$S0_rng[2])))
		
	S1<-as.matrix(cbind(
		rtriangle(input$nyears,input$S1_rng[1],input$S1_rng[2],input$S1),
		runif(input$nyears,input$S2_rng[1],input$S2_rng[2])))
	S2<-as.matrix(cbind(
		rtriangle(input$nyears,input$S2_rng[1],input$S2_rng[2],input$S2),
		runif(input$nyears,input$S1_rng[1],input$S1_rng[2])))

	# SURVIVAL AGE-1 TO MAX AGE
    S<- c(S1[1,input$type],S2[1,input$type],rep(S2[1,input$type],input$maxAge-1))
	
	
	# JUVENILES	
	## INITIALIZE NUMBER OF JUVENILES BY ORIGIN	
	juv_ini_n<- as.matrix(cbind(
		rtriangle(1,input$juv_ini_n_rng[1],input$juv_ini_n_rng[2], input$juv_ini_n),
		runif(1,input$juv_ini_n_rng[1],input$juv_ini_n_rng[2])))
		
	juv_ini_h<-as.matrix(cbind(
		rtriangle(1,input$juv_ini_h_rng[1],input$juv_ini_h_rng[2], input$juv_ini_h),
		runif(1,input$juv_ini_h_rng[1],input$juv_ini_h_rng[2])))

	out<- data.frame(origin=c(rep("h",juv_ini_h[1,input$type]), 
		rep("n",juv_ini_n[1,input$type])),sex=NA,age=NA,yr_since_spawn=-1)
	## ASSIGN SEX
	out$sex<- sample(c('m','f'),nrow(out),replace=TRUE,
		prob=c(0.5,0.5))# assumes 50:50 for juveniles
	## ASSIGN AGE
	out$age<- sample(c(1:(input$mat_high-1)),nrow(out),
		replace=TRUE,prob=cumprod(S[1:(input$mat_high-1)]))
	## ASSIGN FORK LENGTH
	...
	adults_ini_n<- as.matrix(cbind(
		rtriangle(1,input$adults_ini_n_rng[1],input$adults_ini_n_rng[2], input$adults_ini_n),
		runif(1,input$adults_ini_n_rng[1],input$adults_ini_n_rng[2])))
	adults_ini_h<-as.matrix(cbind(
		rtriangle(1,input$adults_ini_h_rng[1],input$adults_ini_h_rng[2], input$adults_ini_h),	
		runif(1,input$adults_ini_h_rng[1],input$adults_ini_h_rng[2])))	

	yyy<- data.frame(origin=c(rep("h",adults_ini_h[1,input$type]), 
		rep("n",adults_ini_n[1,input$type])),sex=NA,age=NA,yr_since_spawn=-1)

	# ASSIGN SEX TO INDIVUAL FISH GIVEN INITIAL SEX RATIO
	yyy[yyy$origin=="h",]$sex<- sample(c('m','f'),adults_ini_h[1,input$type],
		replace=TRUE,prob=c(1-input$sr_h,input$sr_h))
	yyy[yyy$origin=="n",]$sex<- sample(c('m','f'),adults_ini_n[1,input$type],
		replace=TRUE,prob=c(1-input$sr_n,input$sr_n))
...
xx_ind<- function(input=input)
	...
	viableGam<- out_vals$viableGam
	S0<- out_vals$S0
	S1<- out_vals$S1
	S2<- out_vals$S2
	
	# SURVIVAL FUNCTION
	surv_fun<- approxfun(c(0,1,2),	c(S0[1,input$type],
		S1[1,input$type],S2[1,input$type]),rule=2)	
	...
	if(length(pop[pop$origin=='n',]$fl)> 100000){eggs<-0}
	## NUMBER OF VIABLE EMBRYOS PRODUCED
	embryos<- qbinom(runif(1),eggs,viableGam[1,input$type]) #rbinom(1,eggs,viableGam)
	# NUMBER OF VIABLE EMBRYOS SURVIVING TO RECRUIT TO AGE-1
	age0<- qbinom(runif(1),embryos,S0[1,input$type]) 
	...
	for(i in 1:input$nyears)
		...
		# SURVIVAL FUNCTION
		surv_fun<- approxfun(c(0,1,2),	c(S0[i,input$type],S1[i,input$type],
			S2[i,input$type]),rule=2)		
		# t+1
		...
		embryos<- qbinom(runif(1),eggs,viableGam[i,input$type]) #rbinom(1,eggs,viableGam)
		# NUMBER OF VIABLE EMBRYOS SURVIVING TO RECRUIT TO AGE-1
		age0<- qbinom(runif(1),embryos,S0[i,input$type]) 
		...
		pop$tmp<- rbinom(nrow(pop),1,
			mat_fun(age=pop$age,mid=input$mat_mid, high=input$mat_high))
		...
		if(age0>0){
			age0_m<-rbinom(1,age0,0.5)		
			recruits<- list(recruits,
				new=data.table(origin='n',sex=c(rep('m',age0_m), 
					rep('f', age0-age0_m)),age=0,
		...
		if(sum(input$age0_stock,input$age1_stock,input$age2_stock+
			...
			age=c(rep(0,rbinom(1,input$age0_stock,0.051)),
				rep(1,input$age1_stock),
				rep(2,input$age2_stock),
				rep(3,input$age3_stock),
				rep(4,input$age4_stock),
				rep(5,input$age5_stock),
				rep(6,input$age6_stock),
				rep(7,input$age7_stock)),
			yr_since_spawn=-1,
			fl=NA,
			p=1,
			surv=1,
		...
		if(nrow(pop)>0)
			{
			app<-data.table(pop[,j=list(abundance=length(fl)),
				by=list(origin,sex,yr_since_spawn)])
			app$year<-i
			}
		xx<- rbindlist(list(xx,app),use.names=TRUE)
		v[i]<-proc.time() - ptm
	...
	err<- xx[,j=list(a=sum(abundance)),by=list(year)]
